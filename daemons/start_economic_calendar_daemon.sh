#!/bin/bash
# ============================================================================
# Economic Calendar Daemon - Start Script
# ============================================================================
# Uruchamia daemon do zarządzania kalendarzem wydarzeń ekonomicznych.
#
# Wydarzenia:
...
#
# Użycie:
#   ./start_economic_calendar_daemon.sh --start    # Uruchom daemon w tle
#   ./start_economic_calendar_daemon.sh --stop    # Zatrzymaj daemon
#   ./start_economic_calendar_daemon.sh --status  # Sprawdź status
#   ./start_economic_calendar_daemon.sh --once     # Jeden cykl (bez tła)
# ============================================================================
...
mkdir -p "$LOG_DIR"
PID_FILE="${LOG_DIR}/economic_calendar_daemon.pid"
LOG_FILE="${LOG_DIR}/economic_calendar_daemon.log"
DAEMON_SCRIPT="${SCRIPT_DIR}/economic_calendar_daemon.py"
...
start_daemon() {
    ...
    log_info "Uruchamiam Economic Calendar Daemon..."
...
case "${1:-}" in
    ...
    *)
        if [ $# -eq 0 ]; then
            ...
            echo "Użycie: $0 [--start|--stop|--status|--restart|--once|--update-all]"
            exit 1
        fi
        ;;
esac
