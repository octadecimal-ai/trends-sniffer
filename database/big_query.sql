SELECT DISTINCT
   ...
   (ss.occurrence_time AT TIME ZONE 'UTC -2')::TIME as occurrence_warsaw_time,
    ...
    (SELECT ci.value FROM indicators i JOIN country_indicators ci ON (i.id=ci.indicator_id AND ci.country_id = c.id) WHERE code = 'GOOGLE_TRENDS' AND is_active = TRUE LIMIT 1) AS indicator_google_trends_weight,
    (SELECT ci.value FROM indicators i JOIN country_indicators ci ON (i.id=ci.indicator_id AND ci.country_id = c.id) WHERE code = 'BIT_SHD_PT' AND is_active = TRUE  LIMIT 1) AS indicator_bitcoin_premium_weight,
    (SELECT ci.value FROM indicators i JOIN country_indicators ci ON (i.id=ci.indicator_id AND ci.country_id = c.id) WHERE code = 'FP.CPI.TOTL.ZG' AND is_active = TRUE LIMIT 1) AS indicator_inflation_weight,
    (SELECT ci.value FROM indicators i JOIN country_indicators ci ON (i.id=ci.indicator_id AND ci.country_id = c.id) WHERE code = 'HASHRATE_SHARE' AND is_active = TRUE LIMIT 1) AS indicator_hashrate_share_weight,
    (SELECT ci.value FROM indicators i JOIN country_indicators ci ON (i.id=ci.indicator_id AND ci.country_id = c.id) WHERE code = 'IT.NET.USER.ZS' AND is_active = TRUE LIMIT 1) AS indicator_internet_access_weight,
    (SELECT ci.value FROM indicators i JOIN country_indicators ci ON (i.id=ci.indicator_id AND ci.country_id = c.id) WHERE code = 'NY.GDP.MKTP.CD' AND is_active = TRUE LIMIT 1) AS indicator_gdp_weight,
    (SELECT ci.value FROM indicators i JOIN country_indicators ci ON (i.id=ci.indicator_id AND ci.country_id = c.id) WHERE code = 'NY.GDP.PCAP.CD' AND is_active = TRUE LIMIT 1) AS indicator_gdp_per_capita_weight,
    (SELECT ci.value FROM indicators i JOIN country_indicators ci ON (i.id=ci.indicator_id AND ci.country_id = c.id) WHERE code = 'SP.POP.TOTL' AND is_active = TRUE LIMIT 1) AS indicator_population_weight,
    (SELECT ci.value FROM indicators i JOIN country_indicators ci ON (i.id=ci.indicator_id AND ci.country_id = c.id) WHERE code = 'SL.UEM.TOTL.ZS' AND is_active = TRUE LIMIT 1) AS indicator_unemployment_weight,
    (SELECT ci.value FROM indicators i JOIN country_indicators ci ON (i.id=ci.indicator_id AND ci.country_id = c.id) WHERE code = 'BIT_SHD_RT' AND is_active = TRUE LIMIT 1) AS indicator_bitcoin_parallel_rate_weight,
